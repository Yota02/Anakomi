...
for filename in files:
    ...
    for old_route, new_route in mappings.items():
        # Regex pour matcher url_for('route'...) ou url_for("route"...)
        # On s'assure de ne pas remplacer si c'est déjà préfixé
        pattern = r"url_for\((['\"])" + re.escape(old_route) + r"(['\"])"
        replacement = r"url_for(\1" + new_route + r"\2"
